...
function AnalyzeTab({ pageAnalysis, ollamaStatus }: { pageAnalysis: PageAnalysis | null; ollamaStatus: OllamaStatus | null }) {
  ...
  async function analyzeCurrentPage() {
    ...
    if (tab?.id) {
      await chrome.runtime.sendMessage({ type: 'ANALYZE_PAGE', payload: { tabId: tab.id } });
    }
...
function AutofillTab({ pageAnalysis }: { pageAnalysis: PageAnalysis | null }) {
  const [fillMode, setFillMode] = useState<'review' | 'copilot'>('review');
  const [isFilling, setIsFilling] = useState(false);
  const [fillProgress, setFillProgress] = useState<{ filled: number; total: number } | null>(null);

  const isAppPage = pageAnalysis?.isApplicationPage;
  const fieldCount = pageAnalysis?.formFields.length ?? 0;

  async function startAutofill() {
    setIsFilling(true);
    setFillProgress({ filled: 0, total: fieldCount });

    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tab?.id) { setIsFilling(false); return; }
    ...
    await chrome.runtime.sendMessage({
      ...
      payload: {
        ...
        jobId: 'current',
        mode: fillMode,
        fieldsToFill: pageAnalysis?.formFields ?? [],
        tabId: tab.id,
      },
    });
      ...
      {isAppPage ? (
        <div className="glass-card" style={{ padding: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
            ...
            <span className="badge badge-info">{fieldCount} fields</span>
          </div>
          <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>
            {pageAnalysis.questions.length} questions · {pageAnalysis.formFields.filter(f => f.mappedProfileField).length} auto-mapped
          </div>

          {/* Progress bar if filling */}
          {isFilling && fillProgress && (
            <div style={{ marginTop: 10 }}>
              <div style={{ height: 4, background: 'var(--color-surface-2)', borderRadius: 2, overflow: 'hidden' }}>
                <div style={{
                  height: '100%',
                  width: `${(fillProgress.filled / fillProgress.total) * 100}%`,
                  background: 'var(--gradient-brand)',
                  borderRadius: 2,
        ...
        <div className="glass-card" style={{ padding: '14px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>No Application Form</div>
          <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>
            Navigate to a job application page to use autofill
          </div>
        </div>
      )}
